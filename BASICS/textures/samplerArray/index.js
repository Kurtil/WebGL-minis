...
void main()
{
    fragColor = texture(u_textures[3], v_texcoord);
}`;

const program = makeProgram(vertexShaderSource, fragmentShaderSource);

const texturesLocation = gl.getUniformLocation(program, "u_textures");
...
setTimeout(() => {
  ...
  gl.texImage2D(
    ...
    gl.RGB,
    gl.UNSIGNED_BYTE,
    DATA_FROM_PIXEL_BUFFER ? 0 : textureData
  );
...
/*** POSITION ***/
makeBuffer(new Float32Array([-0.5, -0.5, 0.5, -0.5, 0, 0.5]));

const positionLocation = gl.getAttribLocation(program, "position");
gl.enableVertexAttribArray(positionLocation);
gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);

makeBuffer(new Float32Array([0, 0, 1, 0, 0.5, 1]));
...
gl.uniform1iv(texturesLocation, [, , ,textureUnit, , , , ]);
